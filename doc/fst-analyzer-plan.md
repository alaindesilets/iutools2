...
**Done.** `tools/fst/milestone0-smoke-test.lexc`; the whole pipeline works
...
**Done.** `tools/fst/lexicon.lexc` + `phonology.xfscript`. This milestone
...
single suffix — `VnSuffixes`/`NvSuffixes` in `tools/fst/lexicon.lexc` are
...
just those words. Write a small script (`tools/fst/` — same spirit as
...
every batch via `tools/fst/next_affix_priority.py`, not a one-time static
...
`tools/fst/lexicon.lexc`'s own "round 10" comment for the full
...
- **On-device runtime** — real, scoped, buildable work, but entirely
  ...
  milestones above run as a dev-time tool (`tools/fst/`), not inside the
