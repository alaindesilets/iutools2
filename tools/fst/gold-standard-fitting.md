...
at a time** (`doc/fst-analyzer-plan.md`'s stated method). A side effect is
...
> development history (see `git log tools/fst/`). Most carry a comment that
...
(`doc/fst-analyzer-plan.md`, Milestone 2), not gold-fitting — the surface
