...
same data. See [`doc/fst-analyzer-plan.md`](../../doc/fst-analyzer-plan.md)
...
directory** (`tools/fst/`) — they import each other by module name and
...
- **`phonology.xfscript`** — genuinely hand-authored, and not for
  ...
  See [`doc/fst-analyzer-plan.md`](../../doc/fst-analyzer-plan.md)'s "Open
...
`segment_iu.py` is the `tools/fst/` counterpart of the `:cli` command
